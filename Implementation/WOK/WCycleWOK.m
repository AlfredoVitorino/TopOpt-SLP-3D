function [x] = WCycleWOK(An,b,x0,niter1,niter2,ngrids,nv,smoother,P,M,R,perm,opchol,opcond,kv,dens,p,emin,pcgp,str,elem,mr)
% WCycle applies the Multigrid W-Cycle recursively, without constructing the global stiffness matrix. %
...
if(nv == ngrids-1)
    ...
    eH = WCycleWOK(An,rH,zeros(length(rH),1),niter1,niter2,ngrids,nv+1,smoother,P,M,R,perm,opchol,opcond,kv,dens,p,emin,pcgp,str,elem,mr); 
    eH = WCycleWOK(An,rH,eH,niter1,niter2,ngrids,nv+1,smoother,P,M,R,perm,opchol,opcond,kv,dens,p,emin,pcgp,str,elem,mr);
end
eh = P{nv}*eH;
x = x + eh; 
