function [M,nnode] = SmootherMatrix(P,str,kv,x,p,emin,omega,nv,Anbatch,elem,mr)
% SmootherMatrix constructs the smoother iteration matrix when the global stiffness matrix is not explicitly generated. %
...
% OUTPUT: M - smoother iteration matrix. 
...
M = spdiags((1/omega)*M,0,size(M,1),size(M,1));
nnode = size(M,1); 
