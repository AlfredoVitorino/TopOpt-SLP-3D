function y = DiagPrec(str,kv,dens,p,emin,elem,mr)
% DiagPrec generates the diagonal preconditioner without constructing the global stiffness matrix. %
...
% OUTPUT: y - vector that contains the diagonal of the inverse of the global stiffness matrix. 
% ---------- %
...
if(mr.op)
    ...
    kel = kv(1:(3*m+1):(3*m)*(3*m)); % Extracting the diagonal of kv.
end
...
if(elem.type == 1 || elemDeg == 1) % Lagrange elements
    ...
    for el = 1:str.nelem
        if(mr.op) % Multiresolution
            ...
            y(dofs) = y(dofs)+kel(1:(3*m+1):(3*m)*(3*m));
            ...
            y(dofs) = y(dofs)+(kel*(emin+(str.E-emin)*(dens(el)^p)));
        end
